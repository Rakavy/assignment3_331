...
ancestor(Ancestor, Descendant):- superstate(Ancestor, anotherState), ancestor(anotherState, Descendant). 

%8. 

inheritss_transitions(State, List) :- findall(transition(Super, _,_,_,_), superstate(Super, State), List).  
